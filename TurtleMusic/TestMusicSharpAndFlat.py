...
class MyTurtle(turtle.Turtle) :
	...
	def sharpenFlatten(self, x, y) :
		if (self.sharpFlat == 'none'):
			self.sharpFlat = 'sharp'
			self.st()
			self.shape('Sharp2.gif')
			self.stampID = self.stamp()
			self.shape('square')
			self.ht()
		elif (self.sharpFlat == 'sharp'):
			self.sharpFlat = 'flat'
			self.st()
			self.shape('Flat2.gif')
			self.clearStamp(self.stampID)
			self.stampID = self.stamp()
			self.shape('square')
			self.ht()
		else:
...
def populateSharpFlats(h):
	s = -325
	for num in range(9):
		topSharpFlatTurtles.append(MyTurtle())
		topSharpFlatTurtles[num].speed(4)
		topSharpFlatTurtles[num].penup()
		topSharpFlatTurtles[num].shape('square')
		topSharpFlatTurtles[num].shapesize(.4, .4)
		topSharpFlatTurtles[num].setpos(s, (h - (num * 5)))
		topSharpFlatTurtles[num].onclick(topSharpFlatTurtles[num].sharpenFlatten)

	for num in range(9):
		botSharpFlatTurtles.append(MyTurtle())
		botSharpFlatTurtles[num].speed(4)
		botSharpFlatTurtles[num].penup()
		botSharpFlatTurtles[num].shape('square')
		botSharpFlatTurtles[num].shapesize(.4, .4)
		botSharpFlatTurtles[num].setpos(s, (h - ((num + 12) * 5)))
		botSharpFlatTurtles[num].onclick(botSharpFlatTurtles[num].sharpenFlatten)
